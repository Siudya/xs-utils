...
case class SramMeta(dataBits: Int, maskBits: Int, set: Int, colNum: Int, array: Int, latency: Int, mcp: Boolean, dp: Boolean, idx: Int) {
...
object SramMeta {
  def apply(meta: Seq[RAM2MBISTParams], arrays: Seq[Int], depth: Seq[Int]): Seq[SramMeta] = {
    ...
    foundryToMetaMap.flatMap({ case (k, s) =>
      k.toUpperCase match {
...
object GENERIC {
  def apply(meta: Seq[(RAM2MBISTParams, Int, Int)]): Seq[SramMeta] = {
    meta.map({ case (param, array, depth) =>
...
object TSMC28 {
  ...
  def apply(meta: Seq[(RAM2MBISTParams, Int, Int)]): Seq[SramMeta] = {
    meta.flatMap({ case (param, array, depth) =>
      val pmo = patR.findFirstMatchIn(param.sramInst)
