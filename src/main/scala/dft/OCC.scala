...
sealed class OccCtrl extends RawModule {
  ...
  private val shifterCg = Module(new STD_CLKGT_func)
  ...
  shifterCg.io.dft_l3dataram_clk := false.B
  shifterCg.io.dft_l3dataramclk_bypass := false.B
...
class OCC extends BaseInstrument(Seq(
  ...
  private val fcg = Module(new STD_CLKGT_func)
  private val scg = Module(new STD_CLKGT_func)
  ...
  fcg.io.dft_l3dataram_clk := false.B
  fcg.io.dft_l3dataramclk_bypass := false.B

  scg.io.CK := slowClockInjected
  scg.io.TE := ctrl.io.sclkEn
  scg.io.E := ctrl.io.sclkEn
  scg.io.dft_l3dataram_clk := false.B
  scg.io.dft_l3dataramclk_bypass := false.B
