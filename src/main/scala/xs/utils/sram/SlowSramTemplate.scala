...
class SlowSramReq[T <: Data](gen: T, set:Int, way:Int) extends Bundle {
...
class SlowSramResp[T <: Data](gen: T, way:Int) extends Bundle {
  val data = Vec(way, gen)
}
...
class SinglePortSramTemplate[T <: Data](
  ...
  val io = IO(new Bundle{
    val req = Flipped(Decoupled(new SlowSramReq(gen, set, way)))
    val resp = Valid(new SlowSramResp(gen, way))
  ...
  io.resp.bits := ram.io.r.resp.data
  ...
  io.req.ready := Mux(io.req.bits.write, ram.io.w.req.ready, ram.io.r.req.ready)
