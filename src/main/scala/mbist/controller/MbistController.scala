...
class MbistController(node: PipelineBaseNode, algorithm: String = "SMarchCHKBvcd", name: String = "MbistController") extends Module {
  ...
  private val param = MbistControllerParam(sramMetaSeq)
  private val algoName = "xs.utils.mbist.algorithm." + algorithm
  private val dp = node.bd.params.hasDualPort

  private val info = InterfaceInfo(desiredName, param.addrBits, param.maxDataWidth, param.arrayBits, param.maxMaskWidth, dp)
  val io = IO(new Bundle {
    val bap = Flipped(new BapBundle(info))
  ...
  private val algo = Module(Class.forName(algoName)
  ...
  io.bap.end := endReg
  io.bap.failed := RegNext(scoreBoard.io.failed.valid, false.B)
