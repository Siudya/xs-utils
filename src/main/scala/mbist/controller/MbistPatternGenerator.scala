...
class MbistPatternGenerator(param: MbistControllerParam) extends Module {
  ...
  private val patCodeSeq = Seq(PatternCode.pat0, PatternCode.pat5, PatternCode.patA, PatternCode.pat1, PatternCode.patC, PatternCode.patIC)
  ...
  private val widthMask = Cat(Seq.tabulate(param.maxDataWidth)({ idx =>
    val bit = Wire(Bool())
  ...
  private val flippedMask = Cat(Seq.tabulate(param.maxDataWidth)({ idx =>
    val bit = Wire(Bool())
    val maskIdx = (idx.U / sram.seg)(log2Ceil(param.maxMaskWidth) - 1, 0)
    bit := maskShifter(maskIdx)
    bit
  }).reverse)
